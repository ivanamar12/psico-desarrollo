...
<nav class="full-box dashboard-Navbar">
		...
		<div class="container-fluid">
		 ...
		 <!-- modal editar -->
<div class="modal fade" id="representanteEditModal" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Actualizar Representante</h5>
...
$(document).ready(function(){
    var tablaRepresentante = $('#tab-repre').DataTable({
        processing:true,
        serverSide:true,
        ajax:{
            url: "{{ route('representante.index')}}",
        },
        columns:[
            {data: 'id'},
            {data: 'nombre'},
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#tab-repre').DataTable().ajax.reload();
                }
            },
            error: function(xhr) {
...
function editrepresentante(id) {
    $.get('/representante/' + id + '/edit', function(representante) {
        // Asignar los datos al modal
        $('#id').val(representante.id);
        $('#nombre2').val(representante.nombre);
        $('#apellido2').val(representante.apellido);
        $('#ci2').val(representante.ci);
        $('#fecha_nac').val(representante.fecha_nac);
        $('#grado2').val(representante.grado);
        $('#telefono2').val(representante.telefono);
        $('#email2').val(representante.email);
        $('#genero_id2').val(representante.genero_id);
        
        // Asignar los datos de la dirección
		if (representante.direccion) {
    $('#estado_id').val(representante.direccion.estado_id);
    $('#municipio_id').val(representante.direccion.municipio_id);
    $('#parroquia_id').val(representante.direccion.parroquia_id);
    $('#sector2').val(representante.direccion.sector);
}


        $("input[name=_token]").val();
        $('#representanteEditModal').modal('toggle'); // Mostrar el modal
    });
}
</script>

<script>
$('#edit-representante-form').submit(function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                $('#representanteEditModal').modal('hide');
